 ...
 private:
  ...
  void query(KDTreeNode *root,
    ...
    while (!stack.empty()) {
      ...
      if (limit != -1 && int64_t(indices.size()) == limit) {
  ...
  inline T distance(const NDArray<T, 1> &query_point, std::size_t index) const noexcept {
    ...
    else if constexpr (distanceType == KDTreeDistanceType::kEucledian_3d) {
      return EucledianDistanceScalar3d(query_point, index);
    }
  }
  ...
  /**
   * @brief 计算x、y、yaw三个维度的欧几里得距离，yaw具有周期性，每个纬度均需归一化，范围[0~1]
   * 
   * @param query_point 
   * @param index 
   * @return T 
   */
  T EucledianDistanceScalar3d(const NDArray<T, 1> &query_point, std::size_t index)  const noexcept {
    // if (m_points.dim(1)!=3) {
    //   throw std::runtime_error("The dimension of the points is not 3.");
    // }
    T diff0 = query_point[0] - m_points[index][0];
    T diff1 = query_point[1] - m_points[index][1];

    T diff2 = std::abs(query_point[2] - m_points[index][2]);
    diff2 = std::min(diff2, 1-diff2);
