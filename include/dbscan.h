 ...
 */
template<class T, class QueryModel=KDTree<T, KDTreeDistanceType::kEucledian_3d>>
class DBSCAN {
 public:
  static constexpr int UNCLASSIFIED = 0; ///< Constant to represent an unclassified point.
   ...
   * @param n_jobs Number of threads to use for parallel processing.
   */
  DBSCAN(const NDArray<T, 2> &points, T eps, size_t minPts, size_t n_jobs = 1)
    : m_points(points),
      ...
      m_thread_pool(n_jobs<=0?1:n_jobs) {
 ...
 private:
  ...
  void expandCluster(size_t idx) {
    ...
    while (!seeds_vec.empty()) {
      ...
      seeds_vec_cpy.reserve(seeds_vec.size() * 2);
      ...
      auto multi_fut = m_thread_pool.parallelize_loop(0, seeds_vec.size(),
        [this, &seeds_vec, &seeds_vec_cpy, &mutex](size_t start, size_t end) {
          ...
          std::unordered_set<size_t> local_uniq; // Set to ensure unique addition of points to the cluster.

          for (size_t i = start; i < end; ++i) {
            size_t point = seeds_vec[i];

            // Skip already classified points.
            if (m_labels[point] != UNCLASSIFIED && m_labels[point] != NOISY) {
              continue;
            }
